...
import { getBoards, getToolboards } from '@/server/services/mcu';
...
export type FileAction = 'created' | 'overwritten' | 'skipped' | 'error' | 'unchanged';
...
export const serializedPartialConfigFromPrinterDefinition = (def: PrinterDefinition) => {
...
export type MaybePrinterWithResolvedToolhead<T extends boolean> = T extends true
	? PrinterDefinitionWithResolvedToolheads
	: PrinterDefinition;
export type MaybeResolvedToolhead<T extends boolean> = T extends true
	? ToolheadConfiguration<boolean>
	: SerializedToolheadConfiguration;
...
): Promise<ToolheadConfiguration<boolean>> => {
...
export const getPrinters = async <T extends boolean = false>(
	...
	return z.array(resolveToolheads ? PrinterDefinitionWithResolvedToolheads : PrinterDefinition).parse(
		printers.map((p) => {
			const toolheads = resolvedToolheads[p.id];
			return {
				...p,
				defaults: {
					...p.defaults,
...
export const isPrinterCfgInitialized = async () => {
...
export const generateKlipperConfiguration = async <T extends boolean>(
	config: PrinterConfiguration,
	overwriteFiles?: string[],
	skipFiles?: string[],
	...
	const addedFiles = await Promise.all(
		newFiles
			...
			.map(async (f) => {
				...
				const diff = await GitService.diff('/dev/null', `/tmp/ratos-added-new-${timehash}.cfg`);
				return {
					fileName: f.fileName,
					diff: diff,
					exists: f.exists,
	...
	const removedFiles = await Promise.all(
		oldFiles
			...
			.map(async (f) => {
				...
				const diff = await GitService.diff(`/tmp/ratos-removed-old-${timehash}.cfg`, '/dev/null');
				return {
					fileName: f.fileName,
					diff: diff,
					exists: f.exists,
	...
	const changedFiles = await Promise.all(
		newFiles
			...
			.map(async (f) => {
				...
				const diff = await GitService.diff(oldPath, `/tmp/ratos-changed-new-${timehash}.cfg`);
				return {
					fileName: f.fileName,
					diff: diff,
					exists: f.exists,
		...
		newFiles
			...
			.map(async (f) => {
				...
				if (oldFile.diskContent !== oldFile.content) {
					...
					await writeFile(`/tmp/ratos-changed-new-${timehash}.cfg`, f.content);
					diff = await GitService.diff(oldPath, `/tmp/ratos-changed-new-${timehash}.cfg`);
				}
				return {
					fileName: f.fileName,
					diff: diff,
					...
					changedFromConfig:
						oldFile.content !== f.content || (f.lastSavedContent != null && oldFile.content !== f.lastSavedContent),
					order: f.order,
					state: 'changed' as const, // Should be unchanged, but types might require matching
				} satisfies Unpacked<FilesToWriteWithState> as Unpacked<FilesToWriteWithState>;
			}),
	);
